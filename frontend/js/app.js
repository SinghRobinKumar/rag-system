...
function renderSessions() {
    ...
    if (!state.sessions.length) {
        container.innerHTML = '<div style="padding:8px 10px;color:var(--text-muted);font-size:12px;">No conversations yet</div>';
        return;
    }
...
function renderDirectoryTree() {
    ...
    if (!state.directories.length) {
        container.innerHTML = '<div style="padding:6px 8px;color:var(--text-muted);font-size:12px;">No folders yet</div>';
        return;
    }
...
async function switchModel(type, name) {
    try {
        await API.put('/api/settings/model', { model_type: type, model_name: name });
...
function setupEventListeners() {
    ...
    document.getElementById('btn-open-upload-main').addEventListener('click', openUpload);
    document.getElementById('btn-close-upload').addEventListener('click', closeUploadPanel);
    document.getElementById('upload-overlay').addEventListener('click', closeUploadPanel);
